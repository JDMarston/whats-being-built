...
- Default to satellite/aerial imagery so the map works outside the first scraped city.
...
- Always auto-select the newest known local aerial layer for the current viewport and zoom; otherwise keep the global Esri satellite basemap visible.
- Keep user-facing imagery choices simple: Satellite, Satellite + street map, and Street map.
- Use a translucent OSM street-map raster over the aerial layer for Satellite + street map until the app needs dedicated vector road labels.
...
- Keep map provider choices in `src/lib/mapProvider.ts` and local aerial imagery layers in `src/lib/imageryLayers.ts`. This keeps project markers, project JSON, and popup behavior separate from the current MapLibre/free-public basemap implementation so a later Google 3D Tiles, Cesium, Mapbox, or other provider upgrade is less invasive.
- Route app behavior through a small `mapView` provider boundary in `src/lib/mapProvider.ts`. Project markers, user location, imagery toggles, view movement, and 3D toggling should call that boundary instead of directly calling MapLibre APIs from feature code.
- Keep direct MapLibre calls limited to the current provider setup and `src/components/MapView.tsx` until there is a reason to add a premium 3D provider.
