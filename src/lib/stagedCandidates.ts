...
export function reviewPriority(candidate: StagedCandidate): number {
  ...
  if (Number.isFinite(candidate.lat) && Number.isFinite(candidate.lng)) priority += 35;
...
export const stagedCandidateStats: StagedCandidateStats = stagedCandidates.reduce(
  (stats, candidate) => {
    ...
    if (Number.isFinite(candidate.lat) && Number.isFinite(candidate.lng)) stats.geocoded += 1;
